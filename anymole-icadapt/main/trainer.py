...
def freeze_submodules(module):
    #time,fps,out modules
    
    for param in model.model.diffusion_model.time_embed.parameters():
        param.requires_grad = False
    for param in model.model.diffusion_model.fps_embedding.parameters():
        param.requires_grad = False
    for param in model.model.diffusion_model.init_attn.parameters():
        param.requires_grad = False
    #temporal_submodules
    for name, submodule in module.named_modules():
        #if ("Temp" in name) or ("temp" in name) :
        if isinstance(submodule, (TemporalConvBlock, TemporalTransformer)):
            for param in submodule.parameters():
                param.requires_grad = False
                #print(f"Freezing {name}: {type(submodule)}")


def get_parser(**parser_kwargs):
    parser = argparse.ArgumentParser(**parser_kwargs)
    parser.add_argument("--seed", "-s", type=int, default=20240725, help="seed for seed_everything")
...
if __name__ == "__main__":
    ...
    # Calculate the number of frozen parameters initially
    trainable_param = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Initial trainable parameters: {trainable_param}")
    freeze_submodules(model)
    
    #[print(c) for c,_ in model.named_children()]
    #model #trainable, 1438M    ##[print(c) for c,_ in model.model.diffusion_model.named_children()]
    #first_stage_model #notTrainable, 83M
    #cond_stage_model #notTrainable, 354M
    #embedder #notTrainable, 683M
    #image_proj_model, trainable, 48M
    
    trainable_param = sum(p.numel() for p in model.parameters() if  p.requires_grad)
    print(f"Trainable parameters after freezing: {trainable_param}")
